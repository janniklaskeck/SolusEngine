...
// cache font data
struct SFontManager::CachedFont
{
	CachedFont()
		: trueTypeFont(NULL)
	{
...
SFontManager::SFontManager(Atlas* _atlas)
	: m_ownAtlas(false)
	, m_atlas(_atlas)
{
	init();
...
SFontManager::SFontManager(uint16_t _textureSideWidth)
	: m_ownAtlas(true)
...
TrueTypeHandle SFontManager::createTtf(const uint8_t* _buffer, uint32_t _size)
{
...
void SFontManager::destroyTtf(TrueTypeHandle _handle)
{
...
FontHandle SFontManager::createFontByPixelSize(TrueTypeHandle _ttfHandle, uint32_t _typefaceIndex, uint32_t _pixelSize, uint32_t _fontType,
...
FontHandle SFontManager::createScaledFontToPixelSize(FontHandle _baseFontHandle, uint32_t _pixelSize)
{
...
void SFontManager::destroyFont(FontHandle _handle)
{
	BX_ASSERT(isValid(_handle), "Invalid handle used");
...
bool SFontManager::preloadGlyph(FontHandle _handle, const wchar_t* _string)
{
	BX_ASSERT(isValid(_handle), "Invalid handle used");
	CachedFont& font = m_cachedFonts[_handle.idx];
...
bool SFontManager::preloadGlyph(FontHandle _handle, CodePoint _codePoint)
{
	BX_ASSERT(isValid(_handle), "Invalid handle used");
	CachedFont& font = m_cachedFonts[_handle.idx];
...
bool SFontManager::addGlyphBitmap(FontHandle _handle, CodePoint _codePoint, uint16_t _width, uint16_t _height, uint16_t _pitch, float extraScale, const uint8_t* _bitmapBuffer, float glyphOffsetX, float glyphOffsetY)
{
	BX_ASSERT(isValid(_handle), "Invalid handle used");
	CachedFont& font = m_cachedFonts[_handle.idx];
...
const FontInfo& SFontManager::getFontInfo(FontHandle _handle) const
{
	BX_ASSERT(isValid(_handle), "Invalid handle used");
	return m_cachedFonts[_handle.idx].fontInfo;
}

float SFontManager::getKerning(FontHandle _handle, CodePoint _prevCodePoint, CodePoint _codePoint)
{
...
const GlyphInfo* SFontManager::getGlyphInfo(FontHandle _handle, CodePoint _codePoint)
{
...
bool SFontManager::addBitmap(GlyphInfo& _glyphInfo, const uint8_t* _data)
{
